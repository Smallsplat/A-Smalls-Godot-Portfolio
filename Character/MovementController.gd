...
func MovementPhysicsProcess(delta):
	...
	var floor_snap_var = 300 if ((CurrentState(["crouching"]) or (CurrentState(["landing"]) && GetFutureState() == "crouching")) && (ground_raycast.get_collision_normal().x != 0) && character.IsOnFloor())else 10
